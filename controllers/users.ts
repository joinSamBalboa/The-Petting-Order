...
// PUT /profile/:id/review/:id
export const editReview = async (req: { params: { id: string; reviewId: string }; currentUser: { _id: string }; body: any }, res: any) => {
  const { id, reviewId } = req.params
  try {
    const user = await User.findById(id)
    if (!user) throw new Error()
    const reviewToEdit = user.reviews.id(reviewId)
    if (!reviewToEdit) throw new Error('review not found')
    if (!reviewToEdit.owner.equals(req.currentUser._id) && (!user.owner.equals(req.currentUser._id))) throw new Error('Unauthorised')
    await reviewToEdit.updateOne(req.body)
    await user.save()
    return res.status(202).json(user)
  } catch (error) {
    console.log(error)
